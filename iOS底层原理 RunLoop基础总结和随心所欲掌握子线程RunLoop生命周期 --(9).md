上篇文章讲了[runtime的简单应用](https://juejin.im/post/5d31815cf265da1bd605de4f),使用钩子实现了对字典和数组的赋值的校验，顺便随手撸了一个简单的`jsonToModel`,`iOS`除了`runtime`还有一个东西的叫做`runloop`，各位看官老爷一定都有了解，那么今天这篇文章初识一下`runloop`。
